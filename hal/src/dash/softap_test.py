...
class SoftAPTestCase(unittest.TestCase):
    ...
    def find_dash_ap(self):
        start = time.time()
        dashs = []
        dash_re = re.compile("dash-\d+")
        while (not dashs and (time.time()-start)<30):
            networks = scan()
            for ap in networks:
                if dash_re.match(ap['ssid']):
                    dashs.append(ap)

        assert_that(dashs, is_not(empty()), "expected at least one dash soft AP network")
